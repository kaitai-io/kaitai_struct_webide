...
function exportValue(obj, debug: IDebugInfo, path: string[], noLazy?: boolean): IExportedValue {
    //if (!debug) debugger;
    var result = <IExportedValue>{ start: debug && debug.start, end: debug && debug.end, path: path, type: getObjectType(obj) };
