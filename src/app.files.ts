...
class LocalStorageFs implements IFileSystem {
    ...
    getRootNode() { return localforage.getItem<IFsItem>(this.filesKey()).then(x => x || <IFsItem>{ fsType: 'local' }).then(x => this.root = x); }
    setRootNode(newRoot) {
        this.root = newRoot;
        return this.save();
    }
...
$(() => {
    ...
    function getSelectedData() {
        return <IFsItem>ui.fileTree.get_node(ui.fileTree.get_selected()).data;
    }
