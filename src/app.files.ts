...
class FileTreeController {
    constructor(public loadFsItem: (fsItem: IFsItem) => Promise<any>, public compile: (srcYaml: string, kslang: string, debug: true | false | 'both') => any){ }

    genChildNode(obj: IFsItem, fn: string) {
    ...
    init() {
        ...
        $(() => {
            ...
            fileTreeCont.bind("dblclick.jstree", function (event) {
                this.loadFsItem(<IFsItem>ui.fileTree.get_node(event.target).data);
            });
            ...
            ctxAction(uiFiles.cloneKsyFile, e => {
                ...
                fss[fsItem.fsType].get(fsItem.fn).then(content =>
                    fss[fsItem.fsType].put(newFn, content).then(fsItem =>
                        this.addKsyFile('localStorage', newFn.split('/').last(), fsItem)));
            });
        });
    }
}