define(["require", "exports"], function (require, exports) {
    ...
    class ConverterPanel {
        ...
        refresh(dataProvider, offset) {
            if (dataProvider && offset !== -1) {
                ...
                [1, 2, 4, 8].forEach(len => [false, true].forEach(signed => [false, true].forEach(bigEndian => this.panel.find(`.i${len * 8}${len == 1 ? '' : bigEndian ? 'be' : 'le'} .${signed ? 'signed' : 'unsigned'}`).text(numConv(len, signed, bigEndian).toString()))));
    ...
    exports.default = ConverterPanel;
});
