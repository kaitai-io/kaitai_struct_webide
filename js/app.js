...
define(["require", "exports", "./app.layout", "./app.errors", "./app.files", "./app.selectionInput", "./parsedToTree", "./app.jail", "./app.practiceMode"], function (require, exports, app_layout_1, app_errors_1, app_files_1, app_selectionInput_1, parsedToTree_1, app_jail_1, app_practiceMode_1) {
    "use strict";
    ...
    function reparse() {
        ...
        return Promise.all([app_jail_1.jailReady, app_jail_1.inputReady, formatReady]).then(() => {
            ...
            jail.remote.reparse((exportedRoot, error) => {
                ...
                app_layout_1.ui.parsedDataTree = parsedToTree_1.parsedToTree(jsTree, exportedRoot, exports.ksyTypes, e => app_errors_1.handleError(error || e), () => app_layout_1.ui.hexViewer.onSelectionChanged());
    ...
    function loadFsItem(fsItem, refreshGui = true) {
        ...
        return app_files_1.fss[fsItem.fsType].get(fsItem.fn).then(content => {
            if (isKsyFile(fsItem.fn)) {
                ...
                return app_jail_1.jailrun('inputBuffer = args; void(0)', content).then(() => refreshGui ? reparse().then(() => app_layout_1.ui.hexViewer.resize()) : Promise.resolve());
            }
        });
    }
    ...
    $(() => {
        ...
        app_layout_1.ui.hexViewer.onSelectionChanged = () => {
            ...
            refreshConverterPanel(app_layout_1.ui.converterPanel, exports.dataProvider, start);
        };
        app_selectionInput_1.refreshSelectionInput();
        ...
        if (app_layout_1.isPracticeMode) {
            inputReady = loadFsItem({ fsType: 'kaitai', fn: app_layout_1.practiceChall.inputFn, type: 'file' });
