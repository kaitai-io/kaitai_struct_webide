define(["require", "exports", "./LocalStorageFs", "./FsHelper", "./KaitaiFs", "./StaticFs"], function (require, exports, LocalStorageFs_1, FsHelper_1, KaitaiFs_1, StaticFs_1) {
    "use strict";
    var kaitaiRoot = { fsType: 'kaitai' };
    kaitaiFsFiles.forEach(fn => FsHelper_1.default.selectNode(kaitaiRoot, fn));
    var kaitaiFs = new KaitaiFs_1.default(kaitaiRoot);
    var staticFs = new StaticFs_1.default();
    var localFs = new LocalStorageFs_1.default("fs");
    var fss = { local: localFs, kaitai: kaitaiFs, static: staticFs };
    class FileTreeController {
        constructor(loadFsItem, compile) {
            this.loadFsItem = loadFsItem;
        ...
        genChildNode(obj, fn) {
            ...
            return {
                ...
                children: isFolder ? this.genChildNodes(obj) : null,
                data: obj
            };
        }
        genChildNodes(obj) {
            return Object.keys(obj.children).map(k => this.genChildNode(obj.children[k], k));
        }
        refreshFsNodes() {
            ...
            localFs.getRootNode().then(root => {
                ...
                if (root)
                    this.genChildNodes(root).forEach(node => ui.fileTree.create_node(localStorageNode, node));
            });
        }
        isKsyFile(fn) { return fn.toLowerCase().endsWith('.ksy'); }
        addNewFiles(files) {
            return Promise.all(files.map(file => {
                return (this.isKsyFile(file.file.name) ? file.read('text') : file.read('arrayBuffer')).then(content => {
                    return localFs.put(file.file.name, content).then(fsItem => {
                        return files.length === 1 ? this.loadFsItem(fsItem) : Promise.resolve();
                    });
                });
            })).then(this.refreshFsNodes);
        }
        addKsyFile(parent, name, fsItem) {
        ...
        init() {
            ...
            $(() => {
                        ...
                        data: [
                            {
                                ...
                                state: { opened: true },
                                children: [
                                    { text: 'formats', icon: 'glyphicon glyphicon-book', children: this.genChildNodes(kaitaiRoot.children['formats']), state: { opened: true } },
                                    { text: 'samples', icon: 'glyphicon glyphicon-cd', children: this.genChildNodes(kaitaiRoot.children['samples']), state: { opened: true } },
                                ]
                            },
                ...
                }).bind('loaded.jstree', this.refreshFsNodes).jstree(true);
                var uiFiles = {
                ...
                ctxAction(uiFiles.generateParser, e => {
                    ...
                    fss[fsItem.fsType].get(fsItem.fn).then(content => {
                        var compiled = this.compile(content, linkData.kslang, !!linkData.ksdebug);
                ...
                uiFiles.uploadFile.on('click', () => openFilesWithDialog(this.addNewFiles));
                ...
                $('#newKsyModal form').submit(function (event) {
                    ...
                    fss[parentData.fsType].put((parentData.fn ? `${parentData.fn}/` : '') + `${ksyName}.ksy`, `meta:\n  id: ${ksyName}\n  file-extension: ${ksyName}\n`).then(fsItem => {
                        this.addKsyFile(ksyParent, `${ksyName}.ksy`, fsItem);
                        return this.loadFsItem(fsItem);
                    });
                });
                fileTreeCont.bind("dblclick.jstree", function (event) {
                    this.loadFsItem(ui.fileTree.get_node(event.target).data);
                });
                ctxAction(uiFiles.cloneKsyFile, e => {
                    var fsItem = getSelectedData();
                    var newFn = fsItem.fn.replace('.ksy', '_' + new Date().format('Ymd_His') + '.ksy');
                    console.log('newFn', newFn);
                    fss[fsItem.fsType].get(fsItem.fn).then(content => fss[fsItem.fsType].put(newFn, content).then(fsItem => this.addKsyFile('localStorage', newFn.split('/').last(), fsItem)));
                });
            });
        }
    }
